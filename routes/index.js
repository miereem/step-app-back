...
const sequelize = new Sequelize(
  "postgres://s343056:a8x3u3Xyid768O1I@localhost:8933/studs"
);
...
app.post("/places", (req, res) => {
  const { token } = req.cookies;
  const { title, address, addedPhotos, opensAt, closesAt, description } =
    req.body;
...
app.put("/places", async (req, res) => {
  const { token } = req.cookies;
  const { id, title, address, addedPhotos, opensAt, closesAt, description } =
    req.body;
  jwt.verify(token, jwtSecret, {}, async (err, userData) => {
